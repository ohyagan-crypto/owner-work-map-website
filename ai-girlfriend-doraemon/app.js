...
const LANXI_BOT_USERNAME = "OC 角色";
const AGENT_LABELS = { lanxi: "哆啦A夢（OC）", shami: "你（AI女友）" };
const DASHBOARD_ACTIONS = {
  ...
  "force-stop": {
    loading: "停止中",
    idle: "停止",
    success: "已送出強制停止，哆啦A夢（OC）只停 OpenClaw；你（AI女友）會中斷 Telegram 執行中任務。",
...
const dashboardStats = [
  ...
  { label: "監控項目", value: "9", note: "你（AI女友）、哆啦A夢（OC）任務、token 與快照" }
];
...
const completedItems = [
  {
    title: "頂部任務指令與功能鍵",
    status: "已套用",
    summary: "目前任務指令移到儀表板最上方，會跟著你（AI女友）／哆啦A夢（OC）切換；新增卡點救援與強制停止兩個功能鍵。",
    points: ["任務指令最上方顯示", "卡點救援保留作業並嘗試續作", "強制停止獨立成按鈕"]
  },
  {
    title: "雙頁資訊整合與哆啦A夢（OC）任務欄",
    status: "已套用",
    summary: "把主控台與技能入口整理成同一個操作頁；你（AI女友）與哆啦A夢（OC）各自有任務快覽，右欄新增哆啦A夢（OC）獨立任務指令。",
    points: ["你（AI女友）任務與哆啦A夢（OC）任務分開顯示", "哆啦A夢（OC）可吃獨立任務欄位", "技能包改成分類摘要與精簡入口"]
  },
  {
    title: "同步、按鈕與技能包修正",
    status: "已套用",
  ...
  },
  {
    title: "頂端左右雙儀表盤",
    status: "已套用",
    summary: "儀表盤固定放在頁面最上方，你（AI女友）與哆啦A夢（OC）左右分開顯示；手機窄螢幕保留雙欄主狀態，細節移到下方區塊避免擠壓。"
  },
  {
    title: "手機排版與全頁科技底修正",
    status: "已套用",
  ...
  {
    title: "你（AI女友）與哆啦A夢（OC）左右分類",
    status: "已套用",
    summary: "頂部儀表盤改為左欄只放你（AI女友） TGBOT / Telegram / Codex 狀態，右欄只放哆啦A夢（OC） OpenClaw / 自動化 / 快照狀態。",
    points: ["你（AI女友）固定左列", "哆啦A夢（OC）固定右列", "任務、token、監控與卡點依人物分類"]
  },
  {
    title: "移除舊版說明區塊",
    status: "已套用",
  ...
  },
  {
    ...
    points: ["你（AI女友）心跳", "哆啦A夢（OC）進程", "任務佇列", "token 統計", "GitHub Pages 快照"]
  },
  {
    title: "秒級刷新與科技特效",
    status: "已套用",
...
const sopItems = [
  ...
  },
  {
    ...
    steps: ["讀取心跳", "讀取任務佇列", "檢查哆啦A夢（OC）進程", "整理 token 與快照來源"]
  },
  {
    title: "技能路由",
...
const skillCatalog = [
  ...
  },
  {
    ...
    scenario: "需要固定角色如哆啦A夢（OC）、你（AI女友）或其他人物在多次生成中保持一致。",
...
const timeline = [
  ["2026-07-03", "整合主控台與技能入口版面，新增哆啦A夢（OC）獨立任務欄位與頂端哆啦A夢（OC）任務快覽。"],
  ["2026-07-03", "修正前端初始化中斷，讓 runtime-status.json、技能包清單與頂端同步狀態可正常顯示。"],
  ["2026-07-03", "重新同步按鈕移到頂端控制列，點擊時加入短促音效、彩色粒子與同步完成回饋。"],
  ["2026-07-03", "頂端儀表盤改為你（AI女友）左側、哆啦A夢（OC）右側的雙欄總覽，手機版保留左右分開且不擠壓主狀態。"],
  ["2026-07-03", "狀態總控台改成你（AI女友）左列、哆啦A夢（OC）右列，任務、token、監控、卡點與檢查依人物來源分類。"],
  ["2026-07-03", "紅圈內 Live Status 說明文字不再顯示，主介面改成左右分欄，左看你（AI女友）與哆啦A夢（OC），右看狀態與監控。"],
  ...
  ["2026-07-02", "建立你（AI女友） TGBOT 與哆啦A夢（OC） OpenClaw 秒級狀態卡。"]
];
...
const fallbackRuntimeStatus = {
  ...
  heartbeat: { name: "你（AI女友）", ageSeconds: null, activeRequests: null },
  lanxiTaskInstruction: "哆啦A夢（OC） OC 角色 自動化任務與瀏覽器流程監控待同步。",
  openclaw: { name: "哆啦A夢（OC）", botUsername: LANXI_BOT_USERNAME, statusKey: "watch", statusLabel: "狀態待同步", processCount: null, watchdogState: "未取得", currentTaskInstruction: "哆啦A夢（OC） OC 角色 自動化任務與瀏覽器流程監控待同步。" },
  deliverables: [],
  monitors: []
};
...
function lanxiTaskInstruction(status) {
  ...
  const lanxiTaskMonitor = monitors.find((item) => {
    ...
    return id.includes("lanxi-task") || id.includes("openclaw-task") || label.includes("哆啦A夢（OC）任務");
  });
  ...
  return `哆啦A夢（OC） ${status.openclaw?.botUsername || LANXI_BOT_USERNAME} 自動化任務：${status.openclaw?.statusLabel || "狀態待同步"}；${processText}；看門排程 ${watchdogText}。`;
}
...
function renderMonitoring(status) {
  ...
  const shamiCards = renderMonitorCards(shamiMonitors, "你（AI女友）監控");
  const lanxiCards = renderMonitorCards(lanxiMonitors, "哆啦A夢（OC）監控");
