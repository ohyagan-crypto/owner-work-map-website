  ...
  function buildTgbotInstruction(item) {
    ...
    return [
      ...
      "下載檔名：" + fileName,
        ...
        "8. 先執行：node apply-codex-update.mjs --url " + downloadUrl + " --password <客服提供的更新密碼> --target-root <目前這套 Codex 根目錄> --dry-run",
        "9. 乾跑必須確認格式、AES-256-GCM 完整性、SHA256、允許路徑與寫入目標都通過；通過後移除 --dry-run 正式套用同一條指令，密碼仍使用客服提供的更新密碼。",
